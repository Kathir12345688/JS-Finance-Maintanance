...
import { fetchDailyDueCustomers } from '../../services/payments';
...
export default function OwnerDailyDueCustomers() {
  ...
  useEffect(() => {
    const loadDailyDueCustomers = async () => {
      setLoading(true);
      setError('');

      try {
        const response = await fetchDailyDueCustomers();
        setCustomers(normalizeListResponse(response));
      } catch (err) {
        setError(err.response?.data?.detail || 'Unable to load daily due customers.');
      } finally {
        setLoading(false);
      }
    };

    loadDailyDueCustomers();
  }, []);
  ...
  return (
    <div className="container-fluid">
      <div className="mb-4">
        <h2 className="mb-1">Daily Due Customers</h2>
        <p className="text-muted">Customers with pending daily collections.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="alert alert-info">No daily due customers at this time.</div>
      ) : (
        <>
                ...
                {paginatedCustomers.map((customer) => (
                  <tr key={customer.id}>
                    ...
                    <td className="fw-semibold text-info">
                      {formatCurrency(customer.daily_collection_amount)}
                    </td>
