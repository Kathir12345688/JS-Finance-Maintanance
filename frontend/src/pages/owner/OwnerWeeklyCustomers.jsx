...
export default function OwnerWeeklyCustomers() {
  ...
  useEffect(() => {
    const loadCustomers = async () => {
      ...
      try {
        const response = await filterCustomersByType('Weekly');
  ...
  return (
    <div className="container-fluid">
      <div className="mb-4">
        <h2 className="mb-1">Weekly Customers</h2>
        <p className="text-muted">Customers with weekly collection schedule.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="alert alert-info">No weekly customers found.</div>
      ) : (
        <>
              ...
              <tbody>
                {paginatedCustomers.map((customer) => (
                  <tr key={customer.id}>
                    ...
                    <td>{formatCurrency(customer.weekly_collection_amount)}</td>
